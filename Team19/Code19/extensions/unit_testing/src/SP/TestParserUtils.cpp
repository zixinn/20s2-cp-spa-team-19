...
TEST_CASE("ParserUtils - getCallsInProc Test") {
    ...
    std::vector<std::pair<STRING, std::unordered_set<STRING>>> ts{
            {
                    "call me;",
                    { "me" },
            },
    ...
    for (int i = 0; i < ts.size(); ++i) {
        STRING test = std::get<0>(ts[i]);
        std::unordered_set<STRING> ss = std::get<1>(ts[i]);
...
TEST_CASE("ParserUtils - isCyclic Test") {
    ...
    std::vector<std::tuple<STRING, bool, std::unordered_set<STRING>>> ts{
            {
                    "procedure abc { x = x + 1; }",
                    false,
                    {},
    ...
    for (int i = 0; i < ts.size(); ++i) {
        ...
        std::unordered_set<STRING> ss = std::get<2>(ts[i]);
