...
TEST_CASE("Parse Prog - Proc Test") {
    ...
    std::vector<STRING> identifiers{ "star", "moon" };
...
TEST_CASE("Parse Prog - Proc - Keyword Test") {
    ...
    ast::Program* prog = p.parseProgram();
    std::vector<ast::Proc*> procs = prog->getProcedures();
    std::vector<STRING> identifiers{ "procedure", "read", "print", "call", "while",
                                     "if", "then", "else" };
...
TEST_CASE("ParseLexer Proc - Test 1, Test") {
    STRING input = "procedure star{ x = 1 + 2 * 3; while (a > b) { x = 2; } print p; read s; } ;";
    std::vector<std::pair<int, STRING>> tts{
            {1, "(x) = ((1) + ((2) * (3)));"},
    ...
    try {
        ...
        for (int i = 0; i < stmts.size(); ++i) {
            ...
            STRING expect_str = std::get<1>(tt);
