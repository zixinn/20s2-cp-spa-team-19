...
#include "PQL/AffectsBipEvaluator.h"
#include "catch.hpp"
...
TEST_CASE("AffectsBipEvaluator evaluate known known") {
    setupAffectsBip1();
    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"1", "8"}, {}, 2), tempResults1);
    unordered_map<STRING, vector<int>> expected1 = {};
    REQUIRE(b1);
    REQUIRE(tempResults1 == expected1);

    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"3", "8"}, {}, 2), tempResults2);
    ...
    bool b3 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"5", "4"}, {}, 2), tempResults3);
    unordered_map<STRING, vector<int>> expected3 = {};
    REQUIRE(b3);
    REQUIRE(tempResults3 == expected3);

    unordered_map<STRING, vector<int>> tempResults4;
    bool b4 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"6", "4"}, {}, 2), tempResults4);
    unordered_map<STRING, vector<int>> expected4 = {};
    REQUIRE_FALSE(b4);
    REQUIRE(tempResults4 == expected4);
}

TEST_CASE("AffectsBipEvaluator evaluate known synonym") {
    setupAffectsBip1();
    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({ {"a", ASSIGN_} }, Clause("AffectsBip", vector<STRING>{"5", "a"}, { "a" }, 1), tempResults1);
    REQUIRE_FALSE(b1);
    REQUIRE(tempResults1.empty());

    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({ {"s", STMT_} }, Clause("AffectsBip", vector<STRING>{"11", "s"}, { "s" }, 1), tempResults2);
    unordered_set<int> actual2(tempResults2["s"].begin(), tempResults2["s"].end());
    unordered_set<int> expected2{ 5, 8 };
    REQUIRE(b2);
    ...
    setupAffectsBip2();
    unordered_map<STRING, vector<int>> tempResults3;
    bool b3 = AffectsBipEvaluator::evaluate({ {"n", PROGLINE_} }, Clause("AffectsBip", vector<STRING>{"7", "n"}, { "n" }, 1), tempResults3);
    unordered_set<int> actual3(tempResults3["n"].begin(), tempResults3["n"].end());
    unordered_set<int> expected3{ 6 };
    REQUIRE(b3);
    REQUIRE(tempResults3.size() == 1);
    REQUIRE(actual3 == expected3);
}

TEST_CASE("AffectsBipEvaluator evaluate known underscore") {
    setupAffectsBip1();

    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"6", "_"}, {}, 1), tempResults1);
    REQUIRE(b1);
    REQUIRE(tempResults1.empty());

    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"5", "_"}, {}, 1), tempResults2);
    REQUIRE_FALSE(b2);
    REQUIRE(tempResults2.empty());
}

TEST_CASE("AffectsBipEvaluator evaluate synonym known") {
    setupAffectsBip1();
    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({ {"s", STMT_} }, Clause("AffectsBip", vector<STRING>{"s", "1"}, { "s" }, 1), tempResults1);
    REQUIRE_FALSE(b1);
    REQUIRE(tempResults1.empty());

    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({ {"a", ASSIGN_} }, Clause("AffectsBip", vector<STRING>{"a", "5"}, { "a" }, 1), tempResults2);
    unordered_set<int> actual2(tempResults2["a"].begin(), tempResults2["a"].end());
    unordered_set<int> expected2{ 1, 3, 10, 11 };
    REQUIRE(b2);
    REQUIRE(tempResults2.size() == 1);
    REQUIRE(actual2 == expected2);
    ...
    setupAffectsBip2();
    unordered_map<STRING, vector<int>> tempResults3;
    bool b3 = AffectsBipEvaluator::evaluate({ {"n", PROGLINE_} }, Clause("AffectsBip", vector<STRING>{"n", "7"}, { "n" }, 1), tempResults3);
    unordered_set<int> actual3(tempResults3["n"].begin(), tempResults3["n"].end());
    unordered_set<int> expected3{ 4 };
    REQUIRE(b3);
    REQUIRE(tempResults3.size() == 1);
    REQUIRE(actual3 == expected3);
}

TEST_CASE("AffectsBipEvaluator evaluate underscore known") {
    setupAffectsBip1();
    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"_", "10"}, {}, 1), tempResults1);
    REQUIRE(b1);
    REQUIRE(tempResults1.empty());

    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"_", "1"}, {}, 1), tempResults2);
    REQUIRE_FALSE(b2);
    REQUIRE(tempResults2.empty());
}

TEST_CASE("AffectsBipEvaluator evaluate synonym synonym") {
    setupAffectsBip2();
    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({ {"a", ASSIGN_}, {"s", STMT_} }, Clause("AffectsBip", vector<STRING>{"a", "s"}, { "a", "s" }, 0), tempResults1);
    ...
    set<pair<int, int>> expected1 = {{make_pair(5, 4), make_pair(6, 5), 
                                      make_pair(7, 6), make_pair(4, 7)} };
    REQUIRE(b1);
    ...
    setupAffectsBip3();
    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({ {"n", PROGLINE_}, {"n", PROGLINE_} }, Clause("AffectsBip", vector<STRING>{"n", "n"}, { "n", "n" }, 0), tempResults2);
    REQUIRE_FALSE(b2);
    REQUIRE(tempResults2.empty());
}

TEST_CASE("AffectsBipEvaluator evaluate synonym underscore") {
    setupAffectsBip1();

    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({ {"a", ASSIGN_} }, Clause("AffectsBip", vector<STRING>{"a", "_"}, { "a" }, 0), tempResults1);
    ...
    setupAffectsBip3();
    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({ {"n", PROGLINE_} }, Clause("AffectsBip", vector<STRING>{"n", "_"}, { "n" }, 0), tempResults2);
    REQUIRE_FALSE(b2);
    REQUIRE(tempResults2.empty());
}

TEST_CASE("AffectsBipEvaluator evaluate underscore synonym") {
    setupAffectsBip1();

    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({ {"a", ASSIGN_} }, Clause("AffectsBip", vector<STRING>{"_", "a"}, { "a" }, 0), tempResults1);
    unordered_set<int> actual1(tempResults1["a"].begin(), tempResults1["a"].end());
    ...
    setupAffectsBip3();
    unordered_map<STRING, vector<int>> tempResults2;
    bool b2 = AffectsBipEvaluator::evaluate({ {"s", STMT_} }, Clause("AffectsBip", vector<STRING>{"_", "s"}, { "s" }, 0), tempResults2);
    REQUIRE_FALSE(b2);
    REQUIRE(tempResults2.empty());
}

TEST_CASE("AffectsBipEvaluator evaluate underscore underscore") {
    setupAffectsBip1();
    unordered_map<STRING, vector<int>> tempResults1;
    bool b1 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"_", "_"}, {}, 0), tempResults1);
    REQUIRE(b1);
    REQUIRE(tempResults1.empty());

    setupAffectsBip3();
    unordered_map<STRING, vector<int>> tempResults3;
    bool b3 = AffectsBipEvaluator::evaluate({}, Clause("AffectsBip", vector<STRING>{"_", "_"}, {}, 0), tempResults3);
    REQUIRE_FALSE(b3);
