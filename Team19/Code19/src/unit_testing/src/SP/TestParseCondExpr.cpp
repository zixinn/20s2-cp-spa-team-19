...
TEST_CASE("ParseLexer CondExpr - Test") {
    ...
    std::vector<std::tuple<STRING, STRING, std::unordered_set<STRING>, std::unordered_set<STRING>>> tests{
            {
                    "(!(4 == 5))",
                    "( ! ( 4 == 5 ) )",
                    {
...
TEST_CASE("ParseLexer CondExpr - Keywords Test") {
    //STRING input = "if = v + x * y + z * t;";    // if not implemented yet
    std::vector<std::tuple<STRING, std::unordered_set<STRING>, std::unordered_set<STRING>>> tests{
            {
                    "(!(4 == call))",
                    {
                            "call",
