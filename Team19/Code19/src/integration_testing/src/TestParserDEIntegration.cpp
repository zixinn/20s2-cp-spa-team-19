...
TEST_CASE("Simple - Parser-DE Test") {
    ...
    REQUIRE(PKB::modifies->getStmtsModifies(varID1) == unordered_set<StmtNum>{ });
}
...
TEST_CASE("Complicated Expression - Parser-DE Test") {
    ...
    REQUIRE(varID1 == 1);
    REQUIRE(PKB::varTable->getVarName(varID1) == "a");
    VarID varID2 = PKB::varTable->getVarID("pollution");
    REQUIRE(varID2 == 2);
...
TEST_CASE("Nested while-if test - Parser-DE Test") {
    ...
    REQUIRE(varID1 == 1);
    REQUIRE(PKB::varTable->getVarName(varID1) == "a");
    VarID varID2 = PKB::varTable->getVarID("c");
    REQUIRE(varID2 == 3);
    REQUIRE(PKB::varTable->getVarName(varID2) == "c");
    VarID varID3 = PKB::varTable->getVarID("b");
    REQUIRE(varID3 == 4);
    REQUIRE(PKB::varTable->getVarName(varID3) == "b");
    VarID varID4 = PKB::varTable->getVarID("p");
    REQUIRE(varID4 == 5);
    REQUIRE(PKB::varTable->getVarName(varID4) == "p");
    VarID varID5 = PKB::varTable->getVarID("s");
    REQUIRE(varID5 == 6);
    REQUIRE(PKB::varTable->getVarName(varID5) == "s");

    VarID condVarId = PKB::varTable->getVarID("v");
    ...
    // Check Modifies
    REQUIRE(PKB::modifies->getStmtsModifies(varID) == unordered_set<StmtNum>{ 1, 2, 3 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID2) == unordered_set<StmtNum>{ 2, 4, 5 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID3) == unordered_set<StmtNum>{ 2, 4, 6 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID5) == unordered_set<StmtNum>{ 8 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID1) == unordered_set<StmtNum>{ });
...
TEST_CASE("Triple-nested while test - Parser-DE Test") {
    ...
    REQUIRE(condVarId == 2);    // varTable ID starts indexing at 0
    REQUIRE(PKB::varTable->getVarName(condVarId) == "v");
    VarID condVarId2 = PKB::varTable->getVarID("t");
    ...
    // Check Modifies
    REQUIRE(PKB::modifies->getStmtsModifies(varID) == unordered_set<StmtNum>{ 1, 2, 3 }); // Container stmt
    REQUIRE(PKB::modifies->getStmtsModifies(varID1) == unordered_set<StmtNum>{  });
    REQUIRE(PKB::modifies->getStmtsModifies(varID2) == unordered_set<StmtNum>{ 2, 4, 5 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID3) == unordered_set<StmtNum>{ 2, 4, 6, 7 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID5) == unordered_set<StmtNum>{ 9 });
...
TEST_CASE("Triple-nested if test - Parser-DE Test") {
    ...
    REQUIRE(varID3 == 5);
    REQUIRE(PKB::varTable->getVarName(varID3) == "b");
    VarID varID4 = PKB::varTable->getVarID("xor");
    REQUIRE(varID4 == 6);
    REQUIRE(PKB::varTable->getVarName(varID4) == "xor");
    VarID varID5 = PKB::varTable->getVarID("p");
    REQUIRE(varID5 == 7);
    REQUIRE(PKB::varTable->getVarName(varID5) == "p");
    VarID varID6 = PKB::varTable->getVarID("s");
    REQUIRE(varID6 == 8);
    ...
    REQUIRE(PKB::modifies->getStmtsModifies(varID2) == unordered_set<StmtNum>{ 2, 4, 5 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID3) == unordered_set<StmtNum>{ 2, 4, 6, 7 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID4) == unordered_set<StmtNum>{ 2, 4, 6, 8 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID6) == unordered_set<StmtNum>{ 10 });
...
TEST_CASE("Simple Call - Parser-DE Test") {
    ...
    // Check Modifies
    REQUIRE(PKB::modifies->getStmtsModifies(varID) == unordered_set<StmtNum>{ 1, 2 });
    ...
    // Check ProcTable
    ProcID procID1 = PKB::procTable->getProcID("star");
    REQUIRE(PKB::procTable->hasProc("star") == true);
    REQUIRE(procID1 == 0);
    ProcID procID2 = PKB::procTable->getProcID("call");
    ...
    // p's of Call(p,q)
    REQUIRE(PKB::calls->getCallers(1) == unordered_set<ProcID>{ 0 });

    // q's of Call(p,q)
    REQUIRE(PKB::calls->getCallees(0) == unordered_set<ProcID>{ 1 });
}
...
TEST_CASE("Medium Call - Parser-DE Test") {
    ...
    // Check Modifies
    VarID varID_x = PKB::varTable->getVarID("x");
    VarID varID_y = PKB::varTable->getVarID("y");
    VarID varID_z = PKB::varTable->getVarID("z");
    VarID varID_m = PKB::varTable->getVarID("m");
    REQUIRE(PKB::modifies->getStmtsModifies(varID_m) == unordered_set<StmtNum>{ 11, 10, 9, 6, 5, 3, 1 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID_z) == unordered_set<StmtNum>{ 8, 6, 5, 3, 1 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID_y) == unordered_set<StmtNum>{ 7, 4, 3, 1 });
    REQUIRE(PKB::modifies->getStmtsModifies(varID_x) == unordered_set<StmtNum>{ 2, 1 });
    REQUIRE(PKB::modifies->getVarsModifiedByProc(0) == unordered_set<VarID>{ varID_x, varID_y, varID_z, varID_m });
    REQUIRE(PKB::modifies->getVarsModifiedByProc(1) == unordered_set<VarID>{ varID_x, varID_y, varID_z, varID_m });
    REQUIRE(PKB::modifies->getVarsModifiedByProc(2) == unordered_set<VarID>{ varID_y });
    REQUIRE(PKB::modifies->getVarsModifiedByProc(3) == unordered_set<VarID>{ varID_m, varID_z });
    REQUIRE(PKB::modifies->getVarsModifiedByProc(4) == unordered_set<VarID>{ varID_m });

    // Check ProcTable
    ProcID procID1 = PKB::procTable->getProcID("procA");
    REQUIRE(PKB::procTable->hasProc("procA") == true);
    REQUIRE(procID1 == 0);
    ProcID procID2 = PKB::procTable->getProcID("procB");
    REQUIRE(PKB::procTable->hasProc("procB") == true);
    REQUIRE(procID2 == 1);
    ProcID procID3 = PKB::procTable->getProcID("procC");
    REQUIRE(PKB::procTable->hasProc("procC") == true);
    REQUIRE(procID3 == 2);
    ProcID procID4 = PKB::procTable->getProcID("procD");
    REQUIRE(PKB::procTable->hasProc("procD") == true);
    REQUIRE(procID4 == 3);
    ProcID procID5 = PKB::procTable->getProcID("procE");
    ...
    // p's of Call(p,q)
    REQUIRE(PKB::calls->getCallers(1) == unordered_set<ProcID>{ 0 });
    REQUIRE(PKB::calls->getCallers(2) == unordered_set<ProcID>{ 1 });
    REQUIRE(PKB::calls->getCallers(3) == unordered_set<ProcID>{ 1 });
    REQUIRE(PKB::calls->getCallers(4) == unordered_set<ProcID>{ 3 });

    // q's of Call(p,q)
    REQUIRE(PKB::calls->getCallees(0) == unordered_set<ProcID>{ 1 });
    REQUIRE(PKB::calls->getCallees(1) == unordered_set<ProcID>{ 2, 3 });
    REQUIRE(PKB::calls->getCallees(3) == unordered_set<ProcID>{ 4 });

    // p's of Call*(p,q)
    REQUIRE(PKB::calls->getCallersStar(1) == unordered_set<ProcID>{ 0 });
    REQUIRE(PKB::calls->getCallersStar(2) == unordered_set<ProcID>{ 1, 0 });
    REQUIRE(PKB::calls->getCallersStar(3) == unordered_set<ProcID>{ 1, 0 });
    REQUIRE(PKB::calls->getCallersStar(4) == unordered_set<ProcID>{ 3, 1, 0 });

    // q's of Call*(p,q)
    REQUIRE(PKB::calls->getCalleesStar(0) == unordered_set<ProcID>{ 1, 2, 3, 4 });
    REQUIRE(PKB::calls->getCalleesStar(1) == unordered_set<ProcID>{ 2, 3, 4 });
    REQUIRE(PKB::calls->getCalleesStar(3) == unordered_set<ProcID>{ 4 });
}